function goodwordform(k, w, o1, o2, o5) {
  //Корректное склонение окончания слова
...
function newYearDays() {
  //Выводим инфо о ближайших Новых годах
  var date = new Date();
  ...
  if (delta1 > 0)
    document.getElementById("info").innerHTML +=
      "С Нового года " +
      goodwordform(delta1, "прош", "ёл", "ло", "ло") +
      " " +
      delta1 +
      " " +
      goodwordform(delta1, "д", "ень", "ня", "ней") +
      "<br>" +
      "\n";
  var delta2 = Math.abs(delta(Y + 1));
  document.getElementById("info").innerHTML +=
    "До следующего Нового года " +
    goodwordform(delta2, "остал", "ся", "ось", "ось") +
    " " +
    delta2 +
    " " +
    goodwordform(delta2, "д", "ень", "ня", "ней") +
    "<br>" +
    "\n";
}
...
export function currentSchedule() {
  const date = new Date();
  const currentSchedule = store.getters["schedule/WeekSchedule"].filter(
    (i) => i.number === numWeekSep(2021) - 20
  ); //NEED TO REFACTOR IN REAL TIME
  const currentScheduleData = currentSchedule[0].days;
  const days = date.getDate() < 10 ? "0" + date.getDate() : date.getDate();
  const month =
    date.getMonth() < 10 ? `0${date.getMonth() - 2}` : date.getMonth() - 4; //NEED TO REFACTOR
  const currDate = days + "." + month + "." + date.getFullYear();
  const currWeeksSchedule = currentScheduleData.filter(
    (i) => i.date === currDate
  );
